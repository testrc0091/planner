...
import { openDB, seedIfNeeded, getOne, save } from './db.js';
...
export async function applyTheme(hexColor) {
  ...
  root.style.setProperty('--dark-bg',   hslToHex(h, Math.min(s + 0.05, 1), Math.max(l - 0.45, 0.05)));
  root.style.setProperty('--dark-card', hslToHex(h, Math.min(s + 0.02, 1), Math.max(l - 0.35, 0.08)));
